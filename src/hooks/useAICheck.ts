...
export function useAICheck() {
	...
	const checkChapter = useCallback(async (granularity: CheckGranularity, startFrom: number = 0) => {
		...
		if (granularity === "chapter") {
			...
			allLines.forEach((line, i) => { if (line.trim() !== "") { reverseMap[i] = indexMap.length; indexMap.push(i); } });

			// 将原始索引转换为显示索引
			const displayStartFrom = reverseMap[startFrom] ?? startFrom;

			setResults(chapter.id, filteredItems.map((p, i) => ({
				paragraphIndex: indexMap[i],
				originalText: p,
				errors: [],
				status: (i < displayStartFrom ? "done" : "pending") as "done" | "pending",
			})));

			for (let i = displayStartFrom; i < filteredItems.length; i++) {
	...
	const checkSingleLine = useCallback(async (displayIndex: number, setSingleCheckingLine: (v: number | null) => void) => {
		const chapter = chapters[currentChapterIndex];
		if (!chapter) return;

		setSingleCheckingLine(displayIndex);
		const allLines = splitParagraphs(chapter.content);
		const filteredItems = allLines.filter((p) => p.trim() !== "");
		if (displayIndex >= filteredItems.length) { setSingleCheckingLine(null); return; }
		const indexMap: number[] = [];
		allLines.forEach((line, i) => { if (line.trim() !== "") indexMap.push(i); });

		const lineText = filteredItems[displayIndex];
		const existing = useProofreadStore.getState().results[chapter.id];
		if (!existing || existing.length === 0) {
			setResults(chapter.id, filteredItems.map((p, i) => ({ paragraphIndex: indexMap[i], originalText: p, errors: [], status: "done" as const })));
		} else {
			updateParagraphResult(chapter.id, displayIndex, { status: "checking", errors: [] });
		}
		...
		try {
			...
			updateParagraphResult(chapter.id, displayIndex, { errors: parseErrors(extractJSON(reply), chapter.id, displayIndex, lineText), status: "done" });
		} catch (err: unknown) {
			updateParagraphResult(chapter.id, displayIndex, { status: "error", errorMessage: err instanceof Error ? err.message : String(err) });
		} finally {
