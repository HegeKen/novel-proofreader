...
export function buildScriptTTSEnhanceUserPrompt(scriptContent: string, configuredCharacters?: Array<{ name: string; role?: string }>): string {
        // 检查是否有旁白角色
        ...
        return `为以下剧本对话添加情感/音色标注(TTS)。规则：所有行(含旁白)标注角色名。格式：角色名：(标签)内容。场景/动作/转场保持原样。唱歌加(唱歌)。纯文本输出，无markdown。${narratorInstruction}
剧本：
${scriptContent}`;
}
...
export function buildReadingModeTTSEnhanceUserPrompt(
        ...
        contextAfter: string,
        configuredCharacters: Array<{ name: string; aliases: string[]; voice?: string; role?: string; relationTerms?: string[] }>
): string {
        const chars = configuredCharacters.length ? configuredCharacters.map(c => {
                ...
                return `- ${c.name}${roleLabel}${alias}${voice}`;
        }).join('\n') : '无已配置角色';
  ...
  "characters": [
    {
      ...
      "voiceDesignPrompt": "音色设计描述（根据角色性别、年龄、性格、身份和地域背景设计，如：温柔甜美，年轻女性，温婉知性，江南口音，适合表达柔情、羞涩、关切等情感）"
    }
  ],
  "relationships": [
    {
...
/** 音色设计生成系统 prompt */
export const VOICE_DESIGN_SYSTEM_PROMPT = `你是一位专业的声音设计师和配音导演。根据提供的角色信息，为该角色生成适合文本转语音(TTS)使用的音色设计描述。

## 输出格式
请直接返回音色设计描述文本，不需要任何额外解释。

## 音色设计维度
请从以下维度进行描述：
1. 音色特质：如温暖、清冷、沙哑、甜美、磁性、沉稳等
2. 年龄感：如少年、青年、中年、老年等
3. 语气风格：如温柔、坚定、俏皮、严肃、慵懒等
4. 情感色彩：适合表达的情感类型
5. 地域特征：如北方口音、南方口音、京腔、吴侬软语等
6. 方言特征：如普通话、东北话、四川话、粤语、上海话等

## 要求
1. 描述要简洁明了，便于TTS系统理解
2. 基于角色的性别、性格、人物设定和地域背景来设计
3. 如果人物小传中提到角色的籍贯、出生地或成长背景，请考虑地域和方言特征
4. 避免使用过于复杂的术语
5. 输出语言与输入一致（中文输入返回中文）

## 示例
输入：角色名：林婉儿，性别：女，角色类型：女主，人物小传：林婉儿是一位温柔善良的江南大家闺秀，知书达理，性格温婉可人。
输出：温柔甜美，年轻女性，温婉知性，江南口音，适合表达柔情、羞涩、关切等情感

输入：角色名：赵铁柱，性别：男，角色类型：男主，人物小传：赵铁柱是一位来自东北的豪爽汉子，性格耿直，说话带着浓重的东北口音。
输出：粗犷豪迈，中年男性，直爽干练，东北口音，适合表达豪爽、热情、直率等情感`;
...
export async function generateVoiceDesign(
	...
	const userPrompt = `请为以下角色生成音色设计描述：

【角色信息】
姓名：${characterInfo.name}
性别：${genderName}
角色类型：${roleName}
${characterInfo.notes ? `人物小传：${characterInfo.notes.slice(0, 500)}` : ""}

请根据上述信息，生成适合TTS的音色设计描述。`;
