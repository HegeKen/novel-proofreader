...
- 🐛 **阅读区搜索弹窗样式修复** - 统一 search-modal 与 config-modal 设计
- 📊 **阅读进度组件重新设计** - 显示章节阅读进度百分比、进度条可视化、预计剩余时间
- 🔍 **全局搜索弹窗优化** - 居中显示，统一遮罩层样式，优化移动端显示
- 📋 **校对队列面板可视化升级** - 卡片式设计，不同状态使用不同颜色
- 📤 **导出功能优化** - 每部小说数据分别导出为单独文件
- 🎯 **error-count 点击跳转** - 点击错误计数快速跳转到第一个未处理错误
- 📌 **proofread-toolbar 固定显示** - 工具栏不再跟随滑动消失
- 🚫 **同步滚动功能移除** - 简化交互，仅保留点击选中后的同步滚动
